	...
	return (
		<Grid container spacing={2}>
					...
					<Card  elevation={5} onClick={() => handleOpen(dayData, weekDaysMap[dayKey])} sx={{ cursor: 'pointer', borderRadius: 2 }}>
						<CardContent>
							<Typography sx={{
								pb: 1,
								textAlign: 'center'
							}} color={'blackColor.main'} variant="h6">{weekDaysMap[dayKey]}</Typography>
							<CalStatistic
			...
			<Dialog open={!!selectedDay} onClose={handleClose}>
				<DialogTitle>
					<Stack>
						<Typography color={'blackColor.main'} sx={{pb: 1, textAlign: 'center'}} variant={'h5'}>{selectedDay?.dayName}</Typography>
						<CalStatistic
						...
						<>
							...
							{Object.entries(selectedDay.dayData.meals).map(([mealType, meals]) => (
								<Stack key={mealType} style={{ marginBottom: 10 }} spacing={1}>
									<Typography color={'blackColor.main'} variant="subtitle1"><b>{mealTypeMap[mealType] || mealType}:</b></Typography>
									{meals.map((meal) => (
							...
							{selectedDay.dayData.workout.map((work, idx) => (
								<Stack key={idx} style={{ marginBottom: 10 }} spacing={1}>
									<Typography color={'blackColor.main'} variant="subtitle1"><b>{work.category}:</b></Typography>
									{work.tasks.map((task) => (
